/* -----------------------------------------------------------------------------
   ...
   Version 2.0 Reworked		22nd Nov 2004
...
for each {1} NO-LOCK:
    ...
    if decLoop MODULO 100 = 0 then status default "dumping data for {1} " + string(decLoop) + " Records".
    ...
    if decLoop = decRecordCount  
        ...
        else put stream MX unformatted "),".
end.
output to "ptmdump.log" append.
put unformatted  decRecordCount " records" skip.
output close.
/* -------------------------------- The end -------------------------------- */
